...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logsigR);
  PARAMETER(logFspr); 		   // log female F commercial current
  ...
  nll -=dnorm(Fspr, mu_Fspr, sd_Fspr, true);
  ...
  // FLAG: not sure how to optimize a single value
  // Here is a place holder for the general idea
  spr_nll = 0 - (spr-target_spr);
